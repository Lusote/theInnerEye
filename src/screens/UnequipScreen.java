...
public class UnequipScreen implements Screen {

	private Creature player;
	private String letters;
	
	public UnequipScreen(){
	...
	public void display(SGPane console) {
		ArrayList<Item> inv = this.getPlayer().getInventory().getEquippedItems();
		Util.clearTextArea();
		Util.printText(0, 0, messages.getString("uqpScrWhatToUnEquip"));
	...
	public Screen reactToInput(KeyEvent key) {
		...
		ArrayList<Item> items = getPlayer().getInventory().getEquippedItems();
	...
	protected boolean isValidItem(Item item) {
		if(item.isEquippable())
			return true;
		this.getPlayer().getDungeon().addMessage(messages.getString("uqpScrUnEquipCanNot"));
		return false;
	}
	
	protected Screen use(Item item) {
		if(isValidItem(item))this.getPlayer().unequip(item);
		return null;
	}
